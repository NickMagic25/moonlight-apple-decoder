...
struct Record {mav_completion c{};uint64_t entry=0;CVPixelBufferRef retained=nullptr;ReplayQosObservation qos;};
...
static NSDictionary* qosObservation(const ReplayQosObservation& value){
    return @{@"observed":@(value.observed),@"requested_class":@(unsigned(value.requested_class)),
        @"requested_class_name":ns(replayQosName(value.requested_class)),@"relative_priority":@(value.relative_priority),
        @"get_status":value.observed?static_cast<id>(@(value.get_status)):NSNull.null};
}
static NSDictionary* qosResult(const ReplayQosExperiment& experiment,const Sink& sink){
    std::map<std::tuple<int,int,int,bool>,uint64_t> counts;
    for(const auto& r:sink.records)++counts[{int(r.qos.requested_class),r.qos.relative_priority,r.qos.get_status,
                                          bool(r.c.trace.valid&MAV_TRACE_CALLBACK)}];
    NSMutableArray* callbacks=[NSMutableArray array];
    for(const auto& item:counts){auto [requested,relative,status,vtCallback]=item.first;
        [callbacks addObject:@{@"requested_class":@(requested),@"requested_class_name":ns(replayQosName(qos_class_t(requested))),
            @"relative_priority":@(relative),@"get_status":@(status),@"has_vt_callback_timestamp":@(vtCallback),@"count":@(item.second)}];}
    return @{@"mode":ns(experiment.mode),@"requested_class":experiment.setter_attempted?static_cast<id>(@(unsigned(experiment.requested_class))):NSNull.null,
        @"setter_attempted":@(experiment.setter_attempted),@"setter_status":experiment.setter_attempted?static_cast<id>(@(experiment.setter_status)):NSNull.null,
        @"before":qosObservation(experiment.before),@"start":qosObservation(experiment.start),@"end":qosObservation(experiment.end),
        @"completion_thread_observations":callbacks,
        @"observation_semantics":@"pthread requested QoS, not effective overrides; one getter per public completion after sink timestamp in every mode; Apple-owned threads unchanged"};
}
static void complete(void* context,const mav_completion* c){
    uint64_t entry=mav_monotonic_time_ns();const auto requestedQos=replayReadQos();auto&s=*static_cast<Sink*>(context);std::lock_guard<std::mutex>g(s.lock);
    if(s.records.size()==s.records.capacity()){++s.overflow;return;}
    Record r;r.c=*c;r.entry=entry;r.qos=requestedQos;if(s.correctness&&c->pixel_buffer){if(s.retained>=32){++s.overflow;}else{r.retained=CVPixelBufferRetain(c->pixel_buffer);++s.retained;s.retainedPeak=std::max(s.retainedPeak,s.retained);if(!s.ownershipProbe)s.ownershipProbe=CVPixelBufferRetain(c->pixel_buffer);}}s.records.push_back(r);s.ready.notify_one();
}
...
int main(int argc,char**argv){@autoreleasepool{mav_decoder*decoder=nullptr;Sink sink;CVMetalTextureCacheRef cache=nullptr;std::thread consumer;ReferenceStats referenceStats;ReplayQosExperiment qos;try{
    std::map<std::string,std::string>o;for(int i=1;i<argc;++i){std::string k=argv[i];if(k=="--help"){std::cout<<"mav-replay --fixture manifest.json --output results/run --mode correctness|paced|throughput|fault [--inflight 2 --fps 120 --loops 1 --warmup 12 --power -1 --queue-depth 16 --seed 7 --jitter-us 0 --drop-every 0 --corrupt-every 0 --reset-every 0 --consumer-delay-ms 0 --loop-mode reset|continuous --qos unchanged|user-initiated|user-interactive]\n";return 0;}if(i+1>=argc)throw std::runtime_error("option missing value");o[k]=argv[++i];}
    auto val=[&](std::string k,std::string d){return o.count(k)?o[k]:d;};qos.select(val("--qos","unchanged"));if(!o.count("--fixture"))throw std::runtime_error("--fixture required");auto m=load(o["--fixture"]);std::string mode=val("--mode","correctness"),out=val("--output","results/replay");
    ...
    if(!qos.apply())throw std::runtime_error("requested QoS could not be applied/read back");
    ...
    status=mav_decoder_drain(decoder);if(status!=MAV_OK)throw std::runtime_error("drain failed");uint64_t end=mav_monotonic_time_ns();qos.finish();mav_metrics metrics{};metrics.struct_size=sizeof(metrics);metrics.version=MAV_ABI_VERSION;mav_decoder_get_metrics(decoder,&metrics);
    ...
    bool qosStable=!qos.end.get_status&&qos.end.requested_class==qos.start.requested_class&&qos.end.relative_priority==qos.start.relative_priority;
    bool passed=qosStable&&(mode=="fault"?outputs>0:!(failures||drops||rejected||displayMismatches||outputs!=expectedOutputs||!metrics.hardware_validated));
    ...
    std::ofstream csv(out+".csv");csv<<"frame_id,generation,status,result,trace_valid,internal_samples,displayed_outputs,show_existing,scheduled_arrival_ns,admission_ns,preparation_start_ns,preparation_end_ns,vt_submit_ns,vt_return_ns,callback_ns,handoff_ns,sink_entry_ns,hardware,bit_depth,pixel_format,completion_qos_class,completion_qos_relative_priority,completion_qos_get_status\n";
