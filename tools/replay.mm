...
int main(int argc,char**argv){@autoreleasepool{mav_decoder*decoder=nullptr;Sink sink;CVMetalTextureCacheRef cache=nullptr;std::thread consumer;ReferenceStats referenceStats;try{
    ...
    std::string skipCapability=val("--skip-capability","0");
    ...
    if(skipCapability!="0"&&skipCapability!="1")throw std::runtime_error("skip-capability must be 0 or 1");
    if(o.count("--skip-capability")&&coldTrace.empty())throw std::runtime_error("skip-capability requires cold-trace");
    // Explicitly override an inherited private setting so CLI default 0 is a
    // reliable same-binary baseline. Other library consumers may use the env.
    if(setenv("MAV_EXPERIMENT_SKIP_CAPABILITY",skipCapability.c_str(),1))throw std::runtime_error("cannot set private capability control");
