...
def main():
    ...
    parser.add_argument('--settings', default=','.join(k for k in SETTINGS if not k.startswith('pool-')))
    ...
    summary = dict(schema_version=1, phase=args.phase, seconds_requested=args.seconds,
                   repetitions=args.repetitions, queue_depth=32, inflight=2, warmup=120,
                   environments=environments, fixtures=fixtures, runs=[],
                   ordering='For each case, rotate setting order each repetition; reverse even repetitions. Runs are serial.',
    ...
    for case in cases:
        ...
        if not applicable:
            continue
        repetitions = args.repetitions if args.phase == 'timed' else 1
        for repetition in range(repetitions):
            ...
            for name in order:
                ...
                command = list(map(str, [binary, '--fixture', fixture['manifest'], '--mode', mode, '--fps', fps,
                                        ...
                                        '--inflight', 2, '--queue-depth', 32, '--output', prefix, *extra]))
                ...
                result = json.loads(prefix.with_suffix('.json').read_text()) if prefix.with_suffix('.json').exists() else {}
                ...
                if args.phase == 'correctness' and process.returncode:
                    raise RuntimeError(f'correctness failed: {prefix.with_suffix(".log")}')
    return int(any(r['exit_code'] for r in summary['runs']))
