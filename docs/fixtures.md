...
```sh
build/mav-fixture --codec av1 --variant hdr10 --width 1920 --height 1080 --fps 120 --frames 120 --gop 60 --bitrate-mbps 50 --output fixtures/generated/example --aomenc .local/aom-build/aomenc
...
Explicit bitrate targets use deterministic, independently changing luma noise below the top one-eighth frame-ID strip, with neutral chroma. This gives the encoder substantially more detail to encode than the default gradient and makes higher bitrate settings meaningful workload inputs. The source hash uses seed `0x6d617631`, coordinates, and frame number; luma stays within video range (16–235 for 8-bit, 64–940 for 10-bit). `generator.content_profile` records `seeded-noise-frame-id-v1` and `generator.content_seed` records the seed. The existing `generator.pattern` identifier preserves the replay tools' frame-ID and spatial-variation checks; it is not a claim that this content profile contains the default gradient. With no explicit bitrate, the original gradient, detail, square, and frame-ID source pixels are unchanged. No filler bytes or padding are used to inflate the measured rate.

For explicit AV1 targets, the encoder uses realtime CPU8 CBR with frame dropping disabled, maximum intra/inter-frame rate of 300%, undershoot/overshoot controls of 10%, and a 1000 ms buffer with 500 ms initial/optimal fullness. For explicit HEVC targets, the requested average bitrate is paired with a one-second hard cap at 120% of that target; the manifest records the exact `[bytes, seconds]` pair in `encoder_settings.data_rate_limits`. These controls reduce short-clip rate excursions, but every case still reports and checks its measured rate independently. The same compressed frame-ID correctness checks apply to both policies, including HDR. Encoder preparation occurs before timed decoder comparisons.
